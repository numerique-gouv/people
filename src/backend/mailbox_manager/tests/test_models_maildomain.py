...
def test_models_maildomains_get_abilities_owner():
    ...
    access = factories.MailDomainAccessFactory(role="owner", user=user)
...
def test_models_maildomains_get_abilities_administrator():
    ...
    access = factories.MailDomainAccessFactory(role="administrator")
    abilities = access.domain.get_abilities(access.user)
    assert abilities == {
        "delete": False,
        "get": True,
...
def test_models_maildomains_get_abilities_member(django_assert_num_queries):
    """Check abilities returned for the member of a mail domain. It's a viewer role."""
    access = factories.MailDomainAccessFactory(role="member")
