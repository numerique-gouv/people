...
def test_api_mail_domain__accesses_create_authenticated_administrator():
    """
    Administrators of a domain should be able to create mail domain accesses except for the "owner" role.
    """
    user = core_factories.UserFactory()
    ...
    new_mail_domain_access = models.MailDomainAccess.objects.filter(user=other_user).get()
    assert response.json() == {
