...
class WebhookSCIMClient:
    ...
    def __getattr__(self, name):
        """Handle calls from webhooks to synchronize created/deleted mailboxes with a distant application."""

        def wrapper(domain, local_part):
            """
            Wrap SCIMClient calls to handle retries, error handling and storing result in the
            calling Webhook instance.
            """
            for webhook in domain.webhooks.all():
                if not webhook.url:
                    continue

                client = SCIMClient()
                status = WebhookStatusChoices.PENDING
                try:
                    response = getattr(client, name)(webhook, local_part)
