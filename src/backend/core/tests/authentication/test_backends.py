...
def test_authentication_getter_new_user_no_email(monkeypatch):
    """
    If no user matches the user's info sub, a user should not be created without email nor SIRET.
    """
    klass = OIDCAuthenticationBackend()

    def get_userinfo_mocked(*args):
        return {"sub": "123"}  # No email, no SIRET
...
def test_authentication_getter_existing_user_via_email_update_organization(
    ...
    assert user.organization.domains == ["my-domain.com"]
