...
def fixture_mock_authorization_server():
    ...
    mock_server = Mock()
    mock_server._url = "https://auth.server.com"
    return mock_server
...
def test_backend_initialization(mock_get_user_model, mock_authorization_server):
    ...
    assert backend._authorization_server == mock_authorization_server
...
def test_introspect_success(
    ...
    resource_server_backend._authorization_server.get_introspection = Mock(
        return_value=jwe
    )
    resource_server_backend._decrypt = Mock(return_value=jws)
    resource_server_backend._authorization_server.import_public_keys = Mock(
    ...
    assert result == jwt
    resource_server_backend._authorization_server.get_introspection.assert_called_once_with(
        "client_id", "client_secret", token
    )
    resource_server_backend._decrypt.assert_called_once_with(
        jwe, private_key="private_key"
    )
    resource_server_backend._authorization_server.import_public_keys.assert_called_once()
...
def test_introspect_introspection_failure(resource_server_backend):
    ...
    token = "invalid_token"
    resource_server_backend._authorization_server.get_introspection.side_effect = (
        HTTPError("Introspection error")
    )
...
def test_introspect_public_key_import_failure(
    ...
    resource_server_backend._authorization_server.import_public_keys.side_effect = (
...
def test_verify_user_info_wrong_scopes(resource_server_backend):
    ...
    expected_message = "Introspection response does not contain all required scopes."
    with patch.object(Logger, "debug") as mock_logger_debug:
        with pytest.raises(SuspiciousOperation, match=expected_message):
            resource_server_backend._verify_user_info(introspection_response)
