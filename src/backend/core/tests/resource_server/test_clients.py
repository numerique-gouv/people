...
def fixture_client():
    ...
    return AuthorizationServerClient(
        ...
        endpoint_jwks="jwks",
        endpoint_introspection="/introspect",
        verify_ssl=True,
        timeout=5,
        proxy=None,
    )
...
def test_authorization_server_client_initialization():
    ...
    new_client = AuthorizationServerClient(
        url="https://auth.example.com/api/v2",
        endpoint_jwks="jwks",
        endpoint_introspection="/checktoken/foo",
        verify_ssl=True,
        timeout=5,
        proxy=None,
    )
...
def test_get_introspection_success(mock_post, client):
    ...
    mock_post.assert_called_once_with(
        ...
        headers=client._introspection_headers,
...
@patch("requests.get")
@pytest.mark.parametrize("invalid_response", [{"keys": [{"foo": "foo"}]}, {}])
def test_import_public_keys_invalid_jwks(mock_get, invalid_response, client):
