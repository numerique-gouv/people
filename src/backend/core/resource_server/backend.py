...
class ResourceServerBackend:
    """Backend as an OAuth 2.0 resource server.
    ...
    def __init__(self, authorization_server):
    ...
    def get_or_create_user(self, access_token, id_token, payload):
        """Maintain API compatibility with OIDCAuthentication class from Mozilla
    ...
    def get_user(self, access_token):
        ...
        This method will introspect the access token to the authorization server,
        to ensure is validity.
    ...
    def _verify_user_info(self, introspection_response):
        """Verify the 'introspection_response' to get valid and relevant user_info.

        The 'introspection_response' should be still active, and while its authentication
    ...
    def _introspect(self, token):
        ...
        try:
            jwe = self._authorization_server.get_introspection(
                self._client_id,
                self._client_secret,
                token,
            )
        ...
        try:
            public_key_set = self._authorization_server.import_public_keys()
