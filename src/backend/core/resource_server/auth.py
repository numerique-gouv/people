...
class ResourceServerAuthentication(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        # TODO: how to authenticate the Service Provider using the received secret?
        access_token, _ = self.decode_authorization_header(request)
        introspection_token = self.introspect(access_token)
    ...
    def introspect(self, access_token):
        """Introspect the access token to determine if it's still valid and active."""
        try:
            encrypted_token = self.retrieve_introspection_token(access_token)
    ...
    def retrieve_introspection_token(self, access_token):
    ...
    def decrypt(self, encrypted_token, private_key):
        ...
        try:
            ...
            raise SuspiciousOperation("Token decryption failed.") from err
    ...
    def decode(self, encoded_token, public_key_set):
        ...
        try:
            token = jwt.decode(
                ...
                public_key_set,
                algorithms=[self.get_settings("OIDC_RS_SIGNING_ALG0")],
            )
        except ValueError as err:
